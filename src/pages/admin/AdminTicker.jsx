...
export default function AdminTicker() {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.BreakingNewsTicker.list("order", 100),
  });

  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["admin", "ticker"] });
  };

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.BreakingNewsTicker.create(data),
    onSuccess: invalidate,
  });
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.BreakingNewsTicker.update(id, data),
    onSuccess: invalidate,
  });
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.BreakingNewsTicker.delete(id),
    onSuccess: invalidate,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <form
        onSubmit={handleAdd}
        className="bg-white border border-border p-5 space-y-3"
      >
            ...
            {[...items]
              ...
              .map((item) => (
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm("¿Eliminar titular?")) {
