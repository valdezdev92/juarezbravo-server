...
export default function AdminDashboard() {
  ...
  const { data: ticker = [] } = useQuery({
    ...
    queryFn: () => base44.entities.BreakingNewsTicker.list("order", 50),
  });
      ...
      <div className="flex items-end justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Resumen general del sitio editorial
          </p>
        </div>
        <Link
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-border">
          <div className="flex items-center justify-between border-b border-border px-5 py-3">
            <h2 className="font-serif font-bold uppercase text-sm tracking-wider">
              Últimos Artículos
            </h2>
            <Link
              to="/admin/articulos"
          ...
          {recent.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">
              ...
              <Link to="/admin/articulos/nuevo" className="text-primary underline">
                Crear el primero
              </Link>
            </div>
          ) : (
            <div className="divide-y divide-border">
              {recent.map((a) => (
                <div
                  key={a.id}
                  className="px-5 py-3 flex items-center gap-3 hover:bg-secondary/40"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {a.is_breaking_news && (
                        <span className="text-[10px] px-2 py-0.5 font-bold uppercase tracking-widest bg-foreground text-white">
                          Urgente
                        </span>
                      )}
                      {a.category && (
                    ...
                    <div className="text-xs text-muted-foreground">
                      {formatRelativeTime(a.created_date)}
                    </div>
                  </div>
                  <Link
        ...
        <div className="bg-white border border-border">
          ...
          {ticker.filter((t) => t.is_active).length === 0 ? (
            <div className="p-6 text-sm text-muted-foreground">
              No hay titulares activos en el ticker.
            </div>
          ) : (
